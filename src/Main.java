...
public class Main {
    public static void main(String[] args) throws CalculatorException {
        ...
        if (primer1.length!=3) {
            throw new CalculatorException("throws Exception //т.к. формат математической операции не удовлетворяет заданию - два операнда и один оператор (+, -, /, *) через пробел");

        }
        try {
            operand1 = Integer.parseInt(primer1[0]);
            if( operand1<0 || operand1>10)
            { throw new CalculatorException("throws Exception //т.к. операнд 1 не входит в промежуток 1-10");
                }
            try {
                operand2 = Integer.parseInt(primer1[2]);
                if (operand2 < 0 || operand2 > 10) {
                    throw new CalculatorException("throws Exception //т.к. операнд 2 не входит в промежуток 1-10");
...
switch (primer1[1]) {
    ...
    case ("/"):
        ...
        break;
    default:
        throw new CalculatorException("throws Exception //т.к. используется оператор, не подходящий в тз");
...
if (answer<=0)
{
    throw new CalculatorException("throws Exception //т.к. римская цифра не может быть меньше или равна 0");
}
    int units = answer%10;
